...
void DynamixelHWInterface::read(ros::Duration &elapsed_time)
{
    ...
    {
      joint_position_[joint_id] = dxl_wb_->convertValue2Radian(dynamixel_ids_[joint_id], dxl_wb_->itemRead(dynamixel_ids_[joint_id], "Present_Position"));
    //  joint_velocity_[joint_id] = dxl_wb_->convertValue2Velocity(dynamixel_ids_[joint_id], dxl_wb_->itemRead(dynamixel_ids_[joint_id], "Present_Velocity"));
    //  joint_effort_[joint_id] = dxl_wb_->convertValue2Torque(dynamixel_ids_[joint_id], dxl_wb_->itemRead(dynamixel_ids_[joint_id], "Present_Load"));
    }
}
